...
def metadata_preprocessing(path_to_metadata, path_to_clinical_data, manifest_path):
    ...
    meta_subset = meta_data.loc[:, [
        "Subject ID", "Number of Images", "File Location"]]
    ...
    def create_row(i, df4, flag=False):
        ...
        while j < 2:
            if not flag:
                if j == 0:
                    file_loc = (
                        str(df3.iloc[i, df3.columns.get_loc("File Location")])
                        + "/1-1.dcm"
                    )
                else:
                    file_loc = (
            ...
            df4 = df4._append(new_row, ignore_index=True)
            #         print(len(df4))
            j += 1
    ...
    for i in tqdm(range(len(df3))):
        ...
        if i == 0:
            ...
            df4 = df4._append(data_to_append, ignore_index=True)
            continue
...
def stratify_data(df, manifest_path):
    ...
    def create_benign_malignant(df, dest_folder):
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            ...
            if any(x in basename for x in matches):  # Check for a 3rd or 4th in path
                ...
                Path(dest_folder + row["subject_id"] + "_b/").mkdir(
                    parents=True, exist_ok=True
                )
            else:
                dest = dest_folder + row["subject_id"] + "/" + basename
                Path(dest_folder + row["subject_id"] + "/").mkdir(
                    parents=True, exist_ok=True
                )
    ...
    def move_dcm_from_subdir(source, destination):
        ...
        for files in files_list:
            ...
            for x in files_list2:
                shutil.move(source + files + "/" + x,
                            destination + str(j) + ".dcm")
                j += 1

    move_dcm_from_subdir(
        source=cmmd_dir + "TRAIN/benign/",
        destination=cmmd_dir + "cmmd_data/TRAIN/benign/",
    )
    move_dcm_from_subdir(
        source=cmmd_dir + "TRAIN/malignant/",
        destination=cmmd_dir + "cmmd_data/TRAIN/malignant/",
    )

    move_dcm_from_subdir(
        source=cmmd_dir + "VAL/benign/", destination=cmmd_dir + "cmmd_data/VAL/benign/"
    )
    move_dcm_from_subdir(
        source=cmmd_dir + "VAL/malignant/",
        destination=cmmd_dir + "cmmd_data/VAL/malignant/",
    )

    move_dcm_from_subdir(
        source=cmmd_dir + "TEST/benign/",
        destination=cmmd_dir + "cmmd_data/TEST/benign/",
    )
    move_dcm_from_subdir(
        source=cmmd_dir + "TEST/malignant/",
    ...
    def convert_dicom_to_png(input_dir, output_dir):
        ...
        classification_list = [
            classification for classification in os.listdir(input_dir)
        ]
        for classification in classification_list:
            print("Working on " + classification + " for \n" + output_dir)
            dicom_list = [
                dcm_image for dcm_image in os.listdir(input_dir + classification)
            ]
            for dcm_image in tqdm(dicom_list):
                ds = pydicom.read_file(
                    input_dir + classification + "/" + dcm_image
                )  # read dicom image
                img = ds.pixel_array  # get image array
                if not os.path.exists(
                    output_dir + classification
                ):  # if file doesnt exist, create it
                    Path(output_dir + classification).mkdir(parents=True,
                                                            exist_ok=True)
                cv2.imwrite(
                    output_dir
                    + classification
                    + "/img_"
                    + dcm_image.replace(".dcm", ".png"),
                    img,
                )  # write png image

    convert_dicom_to_png(
        input_dir=cmmd_dir + "cmmd_data/TRAIN/",
        output_dir=cmmd_dir + "cmmd_data/PNG/TRAIN/",
    )

    convert_dicom_to_png(
        input_dir=cmmd_dir + "cmmd_data/TEST/",
        output_dir=cmmd_dir + "cmmd_data/PNG/TEST/",
    )

    convert_dicom_to_png(
        input_dir=cmmd_dir + "cmmd_data/VAL/",
...
def xception_transfer_learning(train_generator, validation_generator):
    base_model = keras.applications.Xception(
        weights="imagenet", input_shape=(299, 299, 3), include_top=False
    )
...
def xception_fine_tune(train_generator, validation_generator):
    # load model
    model = keras.models.load_model("best_xception_model.keras")
    model.trainable = True

    callbacks = [
        # keras.callbacks.ModelCheckpoint("/media/craig/Henry/ML_models/xception_aug/save_at_{epoch}.h5"),
        keras.callbacks.ModelCheckpoint(
            "best_xception_fine_tuned_model.keras",
            monitor="val_loss",
            mode="min",
            save_best_only=True,
        ),
...
def xception_fine_tune_test(cmmd_data_dir):
    ...
    test_model(model=keras.models.load_model(
        "best_xception_fine_tuned_model.keras"))
    return


def main():
    manifest_path = "tciaDownload/manifest-1616439774456/"
    cmmd_data_dir = "tciaDownload/cmmd_data/"
    path_to_metadata = "tciaDownload/manifest-1616439774456/metadata.csv"
    path_to_clinical_data = "tciaDownload/CMMD_clinicaldata_revision.xlsx"

    print("Preprocessing...")
    curated_metadata = metadata_preprocessing(
        path_to_metadata, path_to_clinical_data, manifest_path
    )

    print("Data stratification...")
    stratify_data(curated_metadata, manifest_path)

    print("Initialising data generators...")
